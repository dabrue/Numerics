...
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  D1MACH, DBESI1, DBSK1E, DCSEVL, INITDS, ZErMsg
...
C***FIRST EXECUTABLE STATEMENT  DBESK1
      ...
      FIRST = .FALSE.
C
      IF (X .LE. 0.D0) CALL ZErMsg ('SLATEC', 'DBESK1',
     +   'X IS ZERO OR NEGATIVE', 2, 2)
      IF (X.GT.2.0D0) GO TO 20
C
      IF (X .LT. XMIN) CALL ZErMsg ('SLATEC', 'DBESK1',
...
C
 20   DBESK1 = 0.D0
      IF (X .GT. XMAX) CALL ZErMsg ('SLATEC', 'DBESK1',
