...
if (mins < -59 .or. mins > 59 ) then
   print*,"Minutes out of range", mins
else if (mins < 0) then
   mins=mins+60
   hours=hours-1
end if
...
if (hours<10) then
   write(hh,fmt=13) hours
else if (hours >= 10 .and. hours <= 99) then
   write(hh,fmt=23) hours
else if (hours > 999) then
   write (hh,"(A3)") "---"
else
   write(hh,fmt=33) hours
endif

   
if (mins<10) then
   write(mm,fmt=12) mins
else 
   write(mm,fmt=22) mins
endif
...
if (msec<10) then
   write(mss,fmt=13) msec
else if (msec >=10 .and. msec <100) then 
   write(mss,fmt=23) msec
else 
   write(mss,fmt=33) msec
endif
