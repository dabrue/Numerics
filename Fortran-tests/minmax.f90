...
real(dp):: x(0:100)
real(dp):: pi,delta
real(dp):: xmin,xmax,minimum,maximum

pi=4.d0*atan(1.d0)
delta=2.d0*pi/100.d0
...
call minmax(A,x,101,3,xmin,xmax,minimum,maximum)

print*,xmin,xmax,minimum,maximum
...
subroutine minmax(array,xvals,numx,k,xmin,xmax,minimum,maximum)
! THIS SUBROUTINE CALCULATES THE MINIMUM AND 
! MAXIMUM VALUE USING A B-SPLINE INTERPOLATION OF
! THE POINTS PROVIDED IN VARIABLE ARRAY

! xvals must be in ascending order

! IMPORTANT: A SPLINE ORDER K GREATER THAN 3 MAY CAUSE RINGING
! IN THE INTERPOLATED FUNCTION
use bspline
implicit none
integer, parameter :: sp=kind(1.0)
integer, parameter :: dp=selected_real_kind(14,300)
integer :: k,numx
integer :: i, j, l, p, q
real(dp)::array(numx),knot(numx+k),bcoef(numx)
real(dp)::xvals(numx)
real(dp)::minimum,maximum,xmin,xmax
real(dp)::tmp,ptmp,deltax,limit
real(dp)::top,bot,test,dtop,dbot
logical :: go=.true.
...
call dbsnak(numx,xvals,k,knot)
call dbsint(numx,xvals,array,k,knot,bcoef)

deltax=(xvals(2)-xvals(1))/10
test=xvals(1)

do while(test.le.xvals(numx))
        tmp=dbsval(test,k,knot,numx,bcoef)
        test=test+deltax
        if (tmp.gt.maximum) then
                maximum=tmp
                xmax=test
        endif
        if (tmp.lt.minimum) then
                minimum=tmp
                xmin=test
        endif
enddo
...
tmp=dbsder(1,xmax,k,knot,numx,bcoef)
print*,tmp

if (tmp.ne.0.d0) then

do i=1,numx-1
        if (xmax.ge.xvals(i).and.xmax.lt.xvals(i+1)) then
                top=xvals(i+1)
                bot=xvals(i-1)
        endif
enddo

do while(abs(tmp).gt.limit)
        test=(top+bot)/2.d0
        tmp=dbsder(1,test,k,knot,numx,bcoef)
        if (tmp*dtop.lt.0.d0.and.tmp*dbot.gt.0.d0) then
                bot=test
        elseif (tmp*dbot.lt.0.d0.and.tmp*dtop.gt.0.d0) then
                top=test
        elseif (dbot.eq.0.d0) then
                tmp=dbot
        elseif (dtop.eq.0.d0) then
                tmp=dtop
        endif
        dtop=dbsder(1,top,k,knot,numx,bcoef)
        dbot=dbsder(1,bot,k,knot,numx,bcoef)
...
tmp=dbsder(1,xmin,k,knot,numx,bcoef)
print*,tmp
if (tmp.ne.0.d0) then

do i=1,numx-1
        if (xmin.ge.xvals(i).and.xmin.lt.xvals(i+1)) then
                top=xvals(i+1)
                bot=xvals(i-1)
        endif
enddo
...
module numeric
  ...
  integer, parameter :: dbl = selected_real_kind(14,300)
